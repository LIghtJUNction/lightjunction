...
- Treat tracked `WORK_REPORT/YYYY/MM/DD.md` content as public-facing by default. Keep daily reports concise and sanitized: record outcomes, decisions, blockers, and next actions, but avoid raw mailbox IDs, private recipient addresses, social record URIs, wallet addresses, exact wallet balances, transaction hashes, internal absolute paths, credential file paths, and other forensic details unless the user explicitly asks for a private incident-style report. Before committing or sending report content, run `uv run scripts/sanitize-work-reports.py` and `uv run scripts/sanitize-work-reports.py --check`.
