...
- Every time the assistant wakes up for any scheduled, proactive, or user-triggered work session, it must read `AGENTS.md` and take one quick look at the assistant mailbox before task-specific work. The assistant mailbox is `lightjunction@agentmail.to`; treat it as a public contact address, but never store the AgentMail API key or other mailbox credentials in tracked files.
