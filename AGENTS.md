...
- Stay on the currently checked-out Git branch by default. Creating or switching to another branch is strictly forbidden unless the user explicitly authorizes that exact action. Convenience, isolation, pull-request preparation, automation defaults, or a tool's preferred workflow are not sufficient reasons to create a branch.
- Before any authorized branch creation or switch, confirm the target repository and branch name with the user. This rule also applies to commands or tools that create branches indirectly, including worktree helpers, pull-request tools, and automated contribution workflows.
