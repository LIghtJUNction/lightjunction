...
function switchApp(appId: AppId): void {
    ...
    document
        .querySelectorAll<HTMLElement>("[data-app-panel]")
        .forEach((panel) => {
...
const commands = defineCommands({
    ...
    projects: async () => {
        ...
        try {
            ...
            loading.remove();
            writeLine(
                `Failed to fetch projects: ${escapeHtml(error instanceof Error ? error.message : String(error))}`,
                "error",
            );
        }
    },
    stats: async () => {
        ...
        try {
            const user = await fetchJson<GitHubUser>(
                "https://api.github.com/users/LIghtJUNction",
            );
            const repos = await fetchJson<Repo[]>(
                "https://api.github.com/users/LIghtJUNction/repos?per_page=100",
            );
            ...
            loading.remove();
            writeLine(
                `Failed to fetch stats: ${escapeHtml(error instanceof Error ? error.message : String(error))}`,
                "error",
            );
        }
    },
    ...
    whoami: () =>
        writeLine(
            "guest<br>LIghtJUNction's digital assistant<br>maintainer of compact tools for real systems",
        ),
...
async function execute(commandLine: string): Promise<void> {
    ...
    if (!handler) {
        writeLine(
            `Command not found: ${escapeHtml(command)}. Try <kbd>help</kbd>.`,
            "error",
        );
        return;
    }
...
const INTERACTIVE_SELECTOR =
    'button, a, textarea, input, select, summary, [contenteditable]:not([contenteditable="false"])';
...
function bindChrome(): void {
    ...
    $("btn-fullscreen").addEventListener(
        "click",
