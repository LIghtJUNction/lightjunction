const GLYPHS = "agent tool memory shell signal route code system verify return ";
...
function insidePortrait(x: number, y: number): boolean {
    const headX = (x - 0.5) / 0.17;
    const headY = (y - 0.27) / 0.21;
    const insideHead = headX * headX + headY * headY <= 1;
    const insideNeck = y >= 0.43 && y <= 0.59 && Math.abs(x - 0.5) < 0.105;
    const shoulderWidth = 0.11 + Math.max(0, y - 0.52) * 0.72;
    const insideBody = y >= 0.52 && y <= 0.96 && Math.abs(x - 0.5) < shoulderWidth;
    return insideHead || insideNeck || insideBody;
}

function makeTarget(random: SeededRandom, width: number, height: number): Point {
...
function makeStart(random: SeededRandom, width: number, height: number): Point {
    ...
    if (edge === 0) return { x: random() * width, y: -random() * height * 0.25 };
    if (edge === 1) return { x: width * (1 + random() * 0.25), y: random() * height };
    if (edge === 2) return { x: random() * width, y: height * (1 + random() * 0.25) };
...
function createParticles(width: number, height: number): GlyphParticle[] {
    ...
    const count = Math.round(Math.min(760, Math.max(330, width * 0.82)));
...
function readInkColor(): string {
    return getComputedStyle(document.documentElement).getPropertyValue("--ink").trim() || "#171411";
}
...
class LetterSwarm {
    ...
    private animate(time: number): void {
        ...
        this.context.font = `${Math.max(8, Math.min(13, this.width / 65))}px ui-monospace, monospace`;
        this.context.textAlign = "center";
        this.context.textBaseline = "middle";

        for (const particle of this.particles) {
            this.moveParticle(particle, time, step);
            this.context.globalAlpha = 0.32 + Math.min(0.5, Math.hypot(particle.velocityX, particle.velocityY) * 0.06);
    ...
    private moveParticle(particle: GlyphParticle, time: number, step: number): void {
        const driftX = Math.sin(time * 0.0007 + particle.phase) * 0.055;
        const driftY = Math.cos(time * 0.0009 + particle.phase) * 0.045;
        particle.velocityX += ((particle.targetX - particle.x) * 0.012 + driftX) * step;
        particle.velocityY += ((particle.targetY - particle.y) * 0.012 + driftY) * step;

        if (this.pointer.active) {
            const dx = particle.x - this.pointer.x;
            const dy = particle.y - this.pointer.y;
            const distance = Math.max(1, Math.hypot(dx, dy));
            const radius = Math.min(130, this.width * 0.2);
            if (distance < radius) {
    ...
    private settleAndDraw(): void {
        ...
        this.particles.forEach((particle) => {
            this.context.fillText(particle.glyph, particle.targetX, particle.targetY);
        });
        this.context.globalAlpha = 1;
    }
}
...
export function initLetterSwarms(): void {
    document.querySelectorAll<HTMLCanvasElement>("[data-letter-swarm]").forEach((canvas) => {
        try {
