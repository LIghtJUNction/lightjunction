...
function createRenderTarget({
    ...
    if (!texture || !framebuffer)
        throw new Error("Unable to create a bloom target.");
...
export class SingularityForgeRenderer {
    ...
    private resize = (): void => {
        ...
        const deviceRatio =
            Math.min(window.devicePixelRatio || 1, 1.2) * qualityScale;
        ...
        this.sceneTarget = createRenderTarget({
            ...targetOptions,
            width,
            height,
        });
        this.bloomA = createRenderTarget({
            ...targetOptions,
            width: bloomWidth,
            height: bloomHeight,
        });
        this.bloomB = createRenderTarget({
            ...targetOptions,
            width: bloomWidth,
            height: bloomHeight,
        });
    ...
    private blur({
        ...
        gl.uniform2f(
            this.blurUniforms.sourceTexel,
            1 / source.width,
            1 / source.height,
        );
        gl.uniform2f(
            this.blurUniforms.outputResolution,
            destination.width,
            destination.height,
        );
    ...
    private renderComposite(): void {
        ...
        gl.uniform2f(
            this.compositeUniforms.resolution,
            this.canvas.width,
            this.canvas.height,
        );
        gl.uniform1f(this.compositeUniforms.time, this.elapsed);
        gl.uniform1f(
            this.compositeUniforms.bloomEnabled,
            this.bloomEnabled ? 1 : 0,
        );
        gl.drawArrays(gl.TRIANGLES, 0, 3);
    }
